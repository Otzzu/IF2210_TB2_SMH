...
public class BoardController {
    ...
    public void setUpOnClickCard(CardComponent card, int finalI, int finalJ) {
        card.setOnMouseClicked(e -> {
            ...
            // First condition for e.getClickCount() == 1
    ...
    private void showPopup(CardComponent cardComponent) {
        Card cardData = cardComponent.getCardData();
        if (cardData instanceof Animal) {
            Animal animal = (Animal) cardData;
            Stage popupStage = new Stage();
            String imagePath = animal.getImage(); // Ensure this method returns a valid path
            HewanPopUp hewanPopUp = new HewanPopUp(popupStage, animal.getName(), animal.getWeight(), animal.getActiveItem().toString(), animal.getCountActivateItem(animal.getActiveItem().toString()), imagePath);
            hewanPopUp.show();
        } else if (cardData instanceof Plant) {
            Plant plant = (Plant) cardData;
            Stage popupStage = new Stage();
            String imagePath = plant.getImage(); // Ensure this method returns a valid path
            TanamanPopup tanamanPopup = new TanamanPopup(popupStage, plant.getName(), plant.getAge(), plant.getActiveItem().toString(), plant.getCountActivateItem(plant.getActiveItem().toString()), imagePath);
            tanamanPopup.show();
        }
    }
